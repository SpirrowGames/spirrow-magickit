# Magickit MCP ツールリファレンス

`CLAUDE.md` から分離した詳細リファレンス。**ツール一覧・パラメータ・使用例・レスポンス形**
はここにある。CLAUDE.md には「コードやツール定義から再導出できないもの」——設計判断・
不変条件・罠——だけを残してある。

各ツールの正確な引数は MCP のツール定義 (docstring) が第一の情報源で、本ファイルは
その上に載る使い方の説明。食い違ったらツール定義が正しい。
...
### GitHub 連携 (`github_dispatch.py`) の使用例
